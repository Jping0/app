function initAudioFn(audioData){
  /**
   * canvasEl
   */
  ...
  var mouseDown = false;
  // var mousePos = { x:180, y:110 };
  var mousePos = { x:0, y:0 };
  // var posFixed = [
  //   {x:parseInt(winW*0.3),y:parseInt(winH*0.3)},
  //   {x:parseInt(winW*0.7),y:parseInt(winH*0.3)},
  //   {x:parseInt(winW*0.7),y:parseInt(winH*0.7)},
  //   {x:parseInt(winW*0.3),y:parseInt(winH*0.7)}
  // ];
  var curPosLine = 1;//当前背景在那条路径上1-上，2-右，3-下，4-左---顺时针旋转
  bgLJ();
  function bgLJ(){
    window.audioBgTime = setInterval(function(){
      if(curPosLine==1){
        // mousePos.x += 10;
  ...
  function initAudiobg() {
    // canvas = document.createElement( 'canvas' );
    canvas = document.getElementById(audioData.canvasEl);
    ...
    // console.log(musicsEl);
    // if(elbox){
    //   var musicsEl = document.querySelector(elbox);
    //   musicsEl.appendChild( canvas );
    //   // musicsEl.addEventListener( 'mousemove', mouseMoveHandler, false );
    // }
  ...
  function animate() {
    // console.log(isStop?'点击了停止':'运行中');
    if(isStop){
      window.cancelAnimationFrame(animateObj);
      clearInterval(window.audioBgTime);
      mousePos = { x:0, y:0 };
      curPosLine = 1;
    }else{
      clearImageData();
  ...
  window.againAnimFrame = function(){
    isStop = false;
    animate();
    bgLJ();
  }
